...
- Generated JTFRAME logical wrappers can wire the public SDRAM pin model directly and expose `apfsim_sdram_*` counters. They should also feed the JTFRAME programming sideband into ROM coverage counters so partial/wrong ROM payloads show as coverage gaps, while ROM-backed missing writes or mismatches remain hard errors. Treat `MEMORY_UNINITIALIZED_READ`, `MEMORY_ROM_WRITE_MISMATCH`, `SDRAM_COMMAND_ERROR`, `MEMORY_BUS_CONTENTION`, and `MEMORY_BYTE_ENABLE_MISMATCH` as blocking for ROM/RAM confidence even if boot/video pass.
