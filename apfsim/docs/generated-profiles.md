...
- Generated JTFRAME logical wrappers instantiate the public SDRAM pin model when SDRAM pins are present, feed JTFRAME programming-sideband ROM coverage into that model, expose `apfsim_sdram_*` activity/corruption counters, and opt the profile into `memory_activity.top_port_classes: ["sdram"]`.
