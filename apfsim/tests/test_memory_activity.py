...
def test_memory_activity_prefers_exact_profile_rom_region(tmp_path):
    root = tmp_path / "core"
    root.mkdir()
    profile_path = tmp_path / "profiles" / "fake.json"
    profile_path.parent.mkdir()
    raw = {
        ...
        "memory": {
            "classes": ["sdram"],
            "external_classes": ["sdram"],
            "models": {"sdram": {"selected": "sdram_pin_model", "confidence": "observed"}},
            "rom_regions": [
                {
                    "slot_id": 1,
                    "file": "game.rom",
    ...
    (artifacts / "result.json").write_text(json.dumps({
        ...
        "memory_activity": {
            ...
            "counters": [
                {"name": "sdram_rom_mismatch_count", "class": "sdram", "value": 1, "error": True, "error_code": "MEMORY_ROM_WRITE_MISMATCH"},
                {"name": "sdram_first_rom_mismatch_addr", "class": "sdram", "value": 6, "error": False},
                {"name": "sdram_first_rom_mismatch_expected", "class": "sdram", "value": 0x1122, "error": False},
                {"name": "sdram_first_rom_mismatch_actual", "class": "sdram", "value": 0x3344, "error": False},
                {"name": "sdram_first_rom_mismatch_dqm", "class": "sdram", "value": 0, "error": False},
            ],
            "errors": [
                {"code": "MEMORY_ROM_WRITE_MISMATCH", "severity": "error", "counter": "sdram_rom_mismatch_count", "class": "sdram", "value": 1, "observed": True},
            ],
    ...
    memory = json.loads((artifacts / "memory_activity.json").read_text())
    event = memory["rom_validation"]["first_error"]
    assert memory["rom_regions"] == raw["memory"]["rom_regions"]
    assert event["source"]["rule"] == "profile_rom_region"
    assert event["source"]["exact"] is True
    assert event["source"]["slot_id"] == 1
    assert event["source"]["file"] == "game.rom"
    assert event["source"]["memory_offset"] == 4
    assert event["source"]["source_offset"] == 0x104
    assert event["byte_lanes"]["lanes"][0]["source_offset"] == 0x104
    assert event["byte_lanes"]["lanes"][1]["source_offset"] == 0x105


def test_rom_region_bank_mismatch_stays_unmatched():
    doc = build_rom_validation(
        {
            "rom_regions": [
                {
                    "slot_id": 1,
            ...
            "counters": [
                {"name": "sdram_rom_coverage_gap_count", "value": 1},
                {"name": "sdram_first_coverage_gap_addr", "value": 1 << 22},
            ],
        },
        {},
    )

    event = doc["first_error"]
    assert event["code"] == "MEMORY_ROM_COVERAGE_GAP"
    assert event["source"]["matched"] is False
    assert event["source"]["rom_regions"][0]["file"] == "game.rom"
